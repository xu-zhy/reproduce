"""
Policy class used for PPO.
author: 
- ProrokLab (https://www.proroklab.org/): Multi-agent Training Logic
- Xuzhy (...): Add Expert Imitation Learning
"""
import logging
import pickle
import random
...
def ppo_surrogate_loss(
    ...
    if policy.config["use_expert"]:
        train_obs = train_batch[SampleBatch.OBS] # (32, 44)
        train_act = train_batch[SampleBatch.ACTIONS] # (32, 8)
        
        expert_indices = torch.randint(0, len(policy.expert_obs_list), (train_obs.shape[0],))
        expert_obs_batch = [policy.expert_obs_list[i] for i in expert_indices]
        expert_act_batch = [policy.expert_act_list[i] for i in expert_indices]
    
        expert_obs = torch.stack(expert_obs_batch, dim=0).to(train_obs.device)
        expert_act = torch.stack(expert_act_batch, dim=0).to(train_obs.device)
        
        expert_preds = policy.expert(expert_obs, expert_act)
        agent_preds = policy.expert(train_obs, train_act)
        
        expert_loss = policy.expert.compute_loss(expert_preds, agent_preds)
        
        policy.expert_optimizer.zero_grad()
        expert_loss.backward()
        policy.expert_optimizer.step()
        
        # reward = agent_preds.log() - (1-agent_preds).log()
        reward = agent_preds.detach()
        # print(reward)
    ...
    if policy.config["use_critic"]:
        ...
        if policy.config["use_expert"]:
            value_fn_out += 0.1 * reward.to(value_fn_out.device)
    else:
...
class MultiPPOTorchPolicy(PPOTorchPolicy, MultiAgentValueNetworkMixin):
    def __init__(self, observation_space, action_space, config):
        ...
        if self.config["use_expert"]:
            self.expert = Expert(
                obs_space=observation_space,
                action_space=action_space,
                hidden_size=64,
            )
            self.expert_optimizer = torch.optim.Adam(self.expert.parameters(), lr=1e-3)
            
            self.expert_obs_list, self.expert_act_list = [], []
            with open("C:\\Coding\\Python\\vmas_test\\reproduce\\buffer_transport.pkl", "rb") as f:
                expert_data = pickle.load(f)
                for batch in expert_data:
                    obs_ = batch["obs"]
                    act_ = batch["actions"]
                    obs_tensor = torch.stack(obs_, dim=0)
                    self.expert_obs_list.append(obs_tensor.view(-1))
                    act_tensor = torch.stack(act_, dim=0)
                    self.expert_act_list.append(act_tensor.view(-1))
